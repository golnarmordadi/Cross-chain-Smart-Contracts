...
use crate::cross_chain_msg::{AcknowledgementMsg, PacketMsg, WhoAmIResponse};
use crate::state::{accounts, AccountData};

pub const CROSS_CHAIN_APP_VERSION: &str = "v1";
...
pub fn ibc_channel_open(_deps: DepsMut, _env: Env, msg: IbcChannelOpenMsg) -> StdResult<()> {
    ...
    if channel.version.as_str() != CROSS_CHAIN_APP_VERSION {
        return Err(StdError::generic_err(format!(
            "Expected channel version to be: `{}`",
            CROSS_CHAIN_APP_VERSION
        )));
    }

    if let Some(counter_version) = msg.counterparty_version() {
        if counter_version != CROSS_CHAIN_APP_VERSION {
            return Err(StdError::generic_err(format!(
...
mod tests {
    ...
    fn connect(mut deps: DepsMut, channel_id: &str) {
        let handshake_open =
            mock_ibc_channel_open_init(channel_id, IbcOrder::Ordered, CROSS_CHAIN_APP_VERSION);
        // first we try to open with a valid handshake
        ibc_channel_open(deps.branch(), mock_env(), handshake_open).unwrap();

        // then we connect (with counter-party version set)
        let handshake_connect =
            mock_ibc_channel_connect_ack(channel_id, IbcOrder::Ordered, CROSS_CHAIN_APP_VERSION);
    ...
    fn enforce_version_in_handshake() {
        ...
        let wrong_order =
            mock_ibc_channel_open_try("channel-12", IbcOrder::Unordered, CROSS_CHAIN_APP_VERSION);
        ...
        let valid_handshake =
            mock_ibc_channel_open_try("channel-12", IbcOrder::Ordered, CROSS_CHAIN_APP_VERSION);
